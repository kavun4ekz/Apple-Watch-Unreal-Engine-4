...
namespace UnrealBuildTool
{
	...
	class RemoteMac
	{
		...
		/// <param name="TargetPlatform">Optional target platform used to select platform-specific remote build settings</param>
		public RemoteMac(FileReference ProjectFile, UnrealTargetPlatform? TargetPlatform = null)
		{
			...
			if(String.IsNullOrEmpty(ServerName))
			{
				...
				if (Ini.GetString("/Script/MacTargetPlatform.MacTargetSettings", "RemoteServerName", out IniServerName) && !String.IsNullOrEmpty(IniServerName))
				{
					this.ServerName = IniServerName;
				}
				else if (GetPlatformString(Ini, TargetPlatform, "RemoteServerName", out IniServerName) && !String.IsNullOrEmpty(IniServerName))
				{
					this.ServerName = IniServerName;
				}
				else
				{
					throw new BuildException("Remote compiling requires a server name. Use the editor (Project Settings > Platforms > Mac > Remote Build) to set up your remote compilation settings.");
				}

				// Parse the username
				string IniUserName;
				if (Ini.GetString("/Script/MacTargetPlatform.MacTargetSettings", "RSyncUsername", out IniUserName) && !String.IsNullOrEmpty(IniUserName))
				{
					this.UserName = IniUserName;
				}
				else if (GetPlatformString(Ini, TargetPlatform, "RSyncUsername", out IniUserName) && !String.IsNullOrEmpty(IniUserName))
				{
					this.UserName = IniUserName;
				}
			}
			...
			if ((Ini.GetString("/Script/MacTargetPlatform.MacTargetSettings", "SSHPrivateKeyOverridePath", out OverrideSshPrivateKeyPath) && !String.IsNullOrEmpty(OverrideSshPrivateKeyPath))
				|| (GetPlatformString(Ini, TargetPlatform, "SSHPrivateKeyOverridePath", out OverrideSshPrivateKeyPath) && !String.IsNullOrEmpty(OverrideSshPrivateKeyPath)))
			{
			...
			GetPlatformString(Ini, TargetPlatform, "BundleIdentifier", out IniBundleIdentifier);
			...
			if (GetPlatformString(Ini, TargetPlatform, "RemoteServerOverrideBuildPath", out RemoteServerOverrideBuildPath) && !String.IsNullOrEmpty(RemoteServerOverrideBuildPath))
			{
		...
		private static bool GetPlatformString(ConfigHierarchy Ini, UnrealTargetPlatform? TargetPlatform, string KeyName, out string Value)
		{
			if (TargetPlatform.HasValue && TargetPlatform.Value == UnrealTargetPlatform.AppleWatch)
			{
				return Ini.GetString("/Script/AppleWatchRuntimeSettings.AppleWatchRuntimeSettings", KeyName, out Value);
			}

			return Ini.GetString("/Script/IOSRuntimeSettings.IOSRuntimeSettings", KeyName, out Value);
		}
